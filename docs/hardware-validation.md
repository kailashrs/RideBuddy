...
6. Share a destination to RideBuddy but do not start it. The cluster should show **GO**; press the handlebar and confirm navigation starts. Then, once guidance is running, confirm the same button skips a waypoint on a multi-stop route and exits on a single-destination one.
