...
6. Expect the handlebar to do nothing while the cluster shows **GO**. The OEM has no inbound path that can start a route — `8280` acts only on skip and exit — so GO is a prompt telling the rider their phone is waiting. If a press *does* start navigation, that is new information: check Diagnostics for an "Unhandled navigation control" entry and record the bytes.
